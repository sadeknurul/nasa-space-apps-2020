...
// Global Variables
mapboxgl.accessToken = 'pk.eyJ1Ijoic2FidWowNzMiLCJhIjoiY2tlcmNxc3dtMDZtdjJ3bDR0c2Zza29ldSJ9.v_rbo_C_Y74lZfHiJxKumQ';
var dataList = [];
var disasterChoice = "";

// Variables
var truckLocation = [90.44081299999999, 23.7031904];
var warehouseLocation = [90.44081299999999, 39.63116809403496];
var lastQueryTime = 0;
...
function getLocation () {
    ...
    if (navigator.geolocation) {
        var get = navigator.geolocation.getCurrentPosition(setLocation);
        return get;
    } else {
...
$(document).ready(function () {
    ...
    $("#report1-submit").click(() => {
        ...
        console.log(text);
        if(text=='Crime Report'){
            $("#modal-reportfor_crime").text(`${text}`);
         $("#modal-select").attr("hidden", true);
         $("#modal-crime").removeAttr("hidden"); 
     }else{
        $("#modal-reportfor").text(`${text}`);
        $("#modal-select").attr("hidden", true);
    ...
    $("#report-submit").click(()=> {
        ...
        $("#modal-report_success").removeAttr("hidden");
    });

        $("#report-submit_crime").click(()=> {
        $("#modal-crime").attr("hidden", true);
        var name = '';
        var phone = '';
        var details = $('#cam-caption_crime').val();
        $.ajax({
            url : "php/action.php",
            method : "POST",
            data : {
    ...
    $("#report-close_crime").click(()=> {
        $("#modal-crime").attr("hidden", true);
    });
    $("#report-close_sucess").click(()=> {
        $("#modal-report_success").attr("hidden", true);
    });
...
$("#report-submit_crime").on('click', function () {
